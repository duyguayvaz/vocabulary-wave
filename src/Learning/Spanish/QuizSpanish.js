...
function QuizSpanish() {
  ...
  const fetchLearnedWords = async () => {
    try {
      ...
      const relationResponse = await axios.get(
        `http://localhost:1337/api/relations?populate=word_id.lang_id&filters[users_id][$eq]=${userId}&&filters[word_id][lang_id][id][$eq]=13&filters[word_status][$eq]=learned`
      );
  ...
  const handleNextWord = () => {
    if (learnedWords.length > 0) {
      ...
      setTimeout(() => navigate('/spanish'), 2000); // 2 saniye sonra bir önceki menüye dön
    }
  };
